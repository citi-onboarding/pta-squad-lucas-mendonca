...
import { overdueButtonConfig } from "@/types/mailTypes";
...
export const BookDetailsModal: React.FC<BookDetailsModalProps> = ({
  ...
  const { getStatus, sendEmail } = useSendOverdueEmail();
                      ...
                      <div key={loan.id} className="border border-gray-200 rounded-lg p-4 flex justify-between items-center bg-white shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="flex items-center gap-3">
                          {currentStatus === 'ATRASADO' && (() =>{
                            const btn = overdueButtonConfig[getStatus(loan.id)];

                            return(
                              <button
                                onClick={() =>
                                  sendEmail(loan.id,{
                                    userEmail: loan.customerEmail,
                                ...
                                className={`px-4 py-1.5 text-sm font-medium rounded transition-colors flex items-center gap-2 ${btn.className}`}
                              >
